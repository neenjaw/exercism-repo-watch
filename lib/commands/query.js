...
exports.handler = async function (argv) {
  try {
    const token = await keytar.getPassword('exercism-watch', 'github-pat')

    if (token === null) {
      throw new Error(
        'Github personal access token not set. Please set a token.'
      )
    }
    ...
    if (confirmAnswer.confirmAdd || confirmAnswer.confirmRemove) {
      status.message('Making requested changes to Exercism subscrptions...')
      status.start()
